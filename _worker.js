...
async function criarPagamento(request) {
  ...
  try {
    ...
    const orderNsu = `ia-trabalho-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;

    const payload = {
      handle: 'medina_pay',
      items: [{ quantity: 1, price: 2790, description: 'IA no Trabalho, Sem Medo (Nem Exagero) - Acesso Vitalício' }],
      order_nsu: orderNsu,
...
async function webhookInfinitePay(request) {
  try {
    ...
    console.log('Venda confirmada (IA no Trabalho):', data.order_nsu, data.paid_amount);
